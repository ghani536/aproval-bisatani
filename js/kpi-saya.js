    ...
    render() {
        ...
        <div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;padding:14px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:12px;">
                <div style="font-size:13px;font-weight:700;color:#334155;"><i class="fas fa-pen" style="color:#7c3aed;"></i> Isi Capaian</div>
        ...
        order.forEach(area => {
            ...
            groups[area].forEach(it => {
                ...
                inputHtml += `<div style="display:flex;gap:10px;align-items:center;padding:8px 0;border-bottom:1px solid #f1f5f9;">
                    ...
                    <input type="number" min="0" step="any" value="${val}" placeholder="0" id="kpi-in-${this._esc(it.id)}"
                        onchange="kpiSaya.saveItem('${this._esc(it.id)}', this.value)"
                        style="width:80px;padding:8px;border:1px solid #cbd5e1;border-radius:8px;text-align:center;font-size:15px;">
                </div>`;
            });
        });
        inputHtml += '<div style="font-size:11px;color:#94a3b8;margin-top:12px;">Isi sesuai capaianmu hari itu. Tersimpan otomatis. Admin akan verifikasi.</div></div>';
